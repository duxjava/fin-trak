...
-- Insert common currencies (only if they don't exist)
...
ALTER TABLE "accounts" ADD COLUMN IF NOT EXISTS "currency_id" integer;

-- Update existing accounts to use USD currency (assuming they were all USD before)
UPDATE "accounts" SET "currency_id" = (SELECT id FROM "currencies" WHERE code = 'USD') WHERE "currency_id" IS NULL;
